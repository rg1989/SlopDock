...
**Phases:** 15–16 | **Plans:** 8 | **Requirements:** 11/11
...
**Success Criteria:**
  ...
  3. User can send Ctrl+C, Ctrl+D, Tab and other terminal control sequences directly to the PTY
  4. The terminal input renders as a fixed-height strip (~3–4 lines tall) at the bottom of the session pane
**Plans**: 3 plans

Plans:
- [x] 15-01-PLAN.md — Wave 0: TerminalInput test scaffold (RED tests for TINPUT-01 through TINPUT-04)
- [x] 15-02-PLAN.md — Wave 1: TerminalInput.tsx component + Terminal.tsx disableStdin prop
- [x] 15-03-PLAN.md — Wave 2: Wire TerminalInput into SessionPane + App.css + human checkpoint
...
Plans:
- [x] 16-01-PLAN.md — Wave 1: ActionBar component + CSS overlay wrapper + SessionPane wiring (ACTION-01, ACTION-02)
- [x] 16-02-PLAN.md — Wave 1: xterm slash interception in TerminalInput + SlashMenu wiring in SessionPane (SLASH-01, SLASH-02)
- [x] 16-03-PLAN.md — Wave 2: AttachBar floating chip strip in SessionPane (ATTACH-01, ATTACH-02)
- [x] 16-04-PLAN.md — Wave 2: Delete Composer.tsx, Composer.test.tsx, and dead CSS (CLEAN-01)
- [x] 16-05-PLAN.md — Wave 3: Human verify all 5 success criteria in browser
...
**Key Accomplishments:**
- xterm.js TerminalInput strip replaces Composer textarea — arrow-key menus, Ctrl sequences, Tab all reach PTY natively
- `disableStdin` prop on display Terminal prevents keystroke theft; TerminalInput owns all input routing
- ActionBar floats as CSS overlay above terminal input — attach, voice, TTS always accessible without obscuring the cursor
- Slash command popup uses `attachCustomKeyEventHandler` + `useRef` booleans to avoid stale-closure bugs
- AttachBar chip strip positioned via `position: absolute; bottom: 100%` above the input wrapper
- Composer.tsx, Composer.test.tsx, and all Composer CSS fully deleted — zero legacy textarea code remains

**Key Decisions:**
- `useRef` booleans (slashOpenRef, inputEmptyRef) inside xterm key handler — React state captured at init time causes stale closures
- All overlays use `position: absolute` on `.terminal-input-wrapper` as the anchor — avoids z-index wars

**Tech Debt:**
- Composer.tsx HMR error still appears in Vite due to a cached module reference — resolves on full reload; minor

**Stats:** 33 files changed, 3,438 insertions, 595 deletions | ~18,700 LOC total | shipped in 1 day
...
| v1.2 Terminal-Native Composer | 15–16 | 8/8 | ✅ Complete | 2026-05-03 |
